...
import { BaseResponse, RequestData } from "../../types/db";
...
const RequestListModal = (data: {
  ...
  const { data: ReqList, mutate } = useSWR<BaseResponse<RequestData[]>>(
    `/post/request/waiting/${data.postIdx}?userIdx=${data.currUserIdx}`,
    fetcher
  );
      ...
      return (
        <RequestLabel>
          ...
          <button
            ...
            onClick={() => onClickAccept(reqData.simp.userIdx)}
          >
            v
          </button>
          <button
            ...
            onClick={() => onClickReject(reqData.simp.userIdx)}
          >
            x
          </button>
        </RequestLabel>
